...
class AsyncEngine:
    ...
    async def async_get_embeddings(self, input_ids: list[list[int]]) -> list[torch.Tensor]:
        ...
        if self.backend != 'turbomind':
            raise ValueError('async_get_embeddings requires the turbomind backend.')
        assert input_ids and all(isinstance(_, list) for _ in input_ids)
    ...
    async def async_get_rerank_scores(
        ...
        # session_len would have max_new_tokens truncated to 0, so no
        # generation step runs and no logits are produced.
        max_input_len = max(1, self.session_len - 1)
        truncated = 0
        _all_input_ids = []
        for ids in all_input_ids:
            if len(ids) > max_input_len:
                truncated += 1
                ids = ids[:max_input_len]
            _all_input_ids.append(ids)
        all_input_ids = _all_input_ids
        if truncated:
            logger.warning(f'[async_get_rerank_scores] truncated {truncated} '
                           f'input(s) to {max_input_len} tokens '
                           f'(session_len={self.session_len})')
        prompt_tokens = sum(len(ids) for ids in all_input_ids)
